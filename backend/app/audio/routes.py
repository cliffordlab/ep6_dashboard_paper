...
def audio_data():
    ...
    y = list(np.random.uniform(100, 200) + 100*np.sin(2*(3.14/np.random.uniform(2, 64))*np.array(x) + np.random.normal(0, 3.14)) + 15*np.random.normal(0, 5, len(x)))
    return {"data": {'x': x,
                     'y': y
                     },
            "stat": {"mean": np.round(np.mean(y), 2),
                     "median": np.round(np.median(y), 2),
                     "variance": np.round(np.std(y), 2),
                     "last10": np.round(np.std(y), 2)
                     }
            }
